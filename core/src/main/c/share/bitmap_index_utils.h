...
struct args {
   uint64_t keys_memory;
   uint64_t keys_memory_size;
   uint64_t values_memory;
   uint64_t values_memory_size;
   uint64_t rows_memory;
   uint64_t rows_memory_size;
   int64_t  key_begin;
   int64_t  key_end;
   int64_t  max_value;
   int64_t  min_value;
   int32_t  partition_index;
   int32_t  block_value_count_mod;
};

struct key_header {
   uint8_t  signature;
   uint64_t sequence;
   uint64_t value_mem_size;
   uint32_t block_value_count;
   uint64_t key_count;
   uint64_t sequence_check;
   uint8_t  padding[27];
} __attribute__((packed));

struct key_entry {
    uint64_t value_count;
    uint64_t first_value_block_offset;
    uint64_t last_value_block_offset;
    uint64_t count_check;
} __attribute__((packed));
